...
calculate_beta0 <- function(x.log, p) {
...
# sim.dataType = "nl-quadratic"
# max.censor.time = 200
# p = 30
# nonzero.genes = NULL#nonzero.genes.list[[as.character(p)]]
# h0 = 1.5
# save_surv_data = F
...
simulate_survival <- function(
  ...
  beta_file <- list.files(surv_path, pattern = glue::glue("beta0-p{p}.csv"), full.names = T)
  ...
  if (sim.dataType == 'linear') {
    ...
    if (is.null(beta0)) beta0 <- calculate_beta0(xtrue.log, p)
  ...
  ## 10 quadratic terms -----------------
    ...
    if (is.null(beta0)) beta0 <- calculate_beta0(xtrue.nl.log, p)
  ...
  if (save_surv_data) {
    ...
    write.csv(surv.out, 
      file.path(surv_path, glue::glue("simSurv_{sim.dataType}-p{p}_full.csv")),
      row.names = F
    )
  }
  ...
  if (save_beta) {
    ...
    write.csv(data.frame(beta0),
      file.path(surv_path, glue::glue("beta0-p{p}.csv"))
    )
  }
  out <- list(
    surv = surv.out,
...
out = simulate_survival(x.true, 'linear', h0=8e-3, p=30, nonzero.genes=nonzero.genes.list[['30']], plot_km = T, save_surv_data = T, surv_folder = "sim_surv")
