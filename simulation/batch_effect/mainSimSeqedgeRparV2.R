...
mainSimSeqedgeRpar=function(beta0, nonzero_position, n_train, n_test, ps_rho_cutoff=0.9, beta_sort_train=0, beta_sort_test=0, batch_size, 
  ...
  for(iter in 1:nsim){
    ...
    xtrue.train=gamma.train*2*10^6  ## scale back to observed counts since pars of gamma dist are estimated from TC scaled original MSK sarcoma data
    ...
    if(he_train==1){
      ...
      x.train=matrix(log(xobs.vec+0.5), nrow=n_train)  ## log scale
      ...
      while(allNA==1){
        ...
        x.train=matrix(log(xobs.vec+0.5), nrow=n_train)
        row.sums.x.train=rowSums(x.train)
    ...
    if(he_test==1){
      ...
      x.test=matrix(log(xobs.vec+0.5), nrow=n_test)  ## log scale
      ...
      while(allNA==1){
        ...
        x.test=matrix(log(xobs.vec+0.5), nrow=n_test)
        row.sums.x.test=rowSums(x.test)
