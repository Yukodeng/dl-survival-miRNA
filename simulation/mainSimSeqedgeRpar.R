...
mainSimSeqedgeRpar=function(beta0, nonzero_position, n_train, n_test, ps_rho_cutoff=0.9, beta_sort_train=0, beta_sort_test=0, batch_size, 
                             he_train, he_test, batch_sd_mu, batch_sd_phi, stratify_train, stratify_test, cmbt_train, cmbt_test, cmbt_frozen, norm_type, 
  ...
  betas.gene=read.csv("betas.gene.csv")$x  ## estimated gene-specific intercept
  libsizes=read.csv("libsizes.csv")$x  ## estimated library size
  disps.gene=read.csv("disps.gene.csv")$x  ## estimated gene-specific dispersion factor
  corr=read.csv("cor.mat.csv")  ## estimated correlation matrix among genes
  geneid=names(corr)
  corr=as.matrix(corr)
  p=length(geneid)
  ...
  for(iter in 1:nsim){
    ...
    xtrue.train=gamma.train[,nonzero_position]*2*10^6  ## scale to observed counts
    lxtrue.train=log2(xtrue.train+0.5)
    h0=1500
    h=h0*exp(lxtrue.train%*%beta0)
    t0=rexp(n_train, h)
    ...
    if(he_train==1){
      batch.n=length(unique(batch.id))
      batch.mu=rnorm(batch.n, 0, batch_sd_mu)
      batch.mu=sort(batch.mu, decreasing=T)
      batch.mu.expand=rep(batch.mu, each=batch_size)
      batch.mu.mat=batch.mu.expand%*%t(rep(1,p))
      if(beta_sort_train==0){
        gamma.train.batch=gamma.train*exp(batch.mu.mat)
        id.new=id
      }else{
    ...
    # generate survival time based on gamma-distributed true gene expression
    xtrue.test=gamma.test[,nonzero_position]*2*10^6  ## scale to observed counts
    lxtrue.test=log2(xtrue.test+0.5)
    h0=0.75
    h=h0*exp(lxtrue.test%*%beta0)
    t0=rexp(n_test, h)
    ct=runif(n_test, 0, 13) 
    ...
    if(he_test==1){
      batch.n=length(unique(batch.id.test))
      batch.mu=rnorm(batch.n, 0, batch_sd_mu)
      batch.mu=sort(batch.mu, decreasing=T)
      batch.mu.expand=rep(batch.mu, each=batch_size)
      batch.mu.mat=batch.mu.expand%*%t(rep(1,p))
      if(beta_sort_test==0){
        ...
        id.new.test[n_test]=id[!(id %in% id.new.test)]
    ...
    if(cmbt_train==1){
      ...
      # write.csv(data_train, "data_train.csv", row.names=F, na="")
      # write.csv(x0_train, "x0_train.csv", row.names=F, na="")
    ...
    error=function(e){iter=iter-1})  ## if there is any error in the entire iteration, the iteration is repeated with a different seed until no error
  }
  cat(" done \n")
