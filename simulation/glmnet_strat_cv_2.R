#### the data must be in the following structure: 1st col-id, 2nd col-t, 3rd col-delta, 4th col-strata, other cols-x
#### in this version 2, cross-validation is based on individual subjects, not batch.

glmnet_strat_cv=function(data, lambda_max, nlambda, nfold, penalty_wt, threshold){
  data=data[order(data[,4]),]
  options(warn=-1)
  temp=as.vector(unlist(by(data[,c("t","delta")], data[,4], function(x){x[,1]<min(x[x[,2]==1,1])}))) ## remove censoring before 1st event
  options(warn=0)
  data=data[!temp,]
  n=nrow(data)
  cvid=sample(rep(1:nfold, each=n/nfold))
        ...
        while(diff>threshold){
          ...
          while(max(change)>threshold){ ###
            ...
            for(k in 1:p){
              xk=x_traincv[,k]
              xnk=x_traincv[,-k]
              numer=mean(xk*(ws*eta_traincv-el)-ws*xk*(xnk%*%bint[-k]))
  ...
  iter2=1
  while(diff>threshold & iter2<200){
    # print("a")
    bintold=bint
    eta=x_std%*%bint
    el=ws=rep(NA,n)
    ...
    while(max(change)>threshold){
      ...
      for(k in 1:p){
        xk=x_std[,k]
        xnk=x_std[,-k]
        numer=mean(xk*(eta*ws-el)-ws*xk*(xnk%*%bint[-k]))
        # print(bint)
        S=sign(numer)*ifelse((abs(numer)-lambda*alpha*penalty_wt[k])>0, abs(numer)-lambda*alpha*penalty_wt[k], 0)
        bhatk=S/(mean(ws*xk^2)+lambda*(1-alpha))
        # print(cat("S is",S))
        # print(cat('lambda is',lambda))
        # print(cat('alpha is',alpha))
        change[k]=abs(bint[k]-bhatk)
        bint[k]=bhatk
      }
    }
