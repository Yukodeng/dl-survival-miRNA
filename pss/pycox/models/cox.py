...
# Import the base and loss modules
from . import base
...
class _CoxBase(base.SurvBase):
    ...
    def compute_baseline_cumulative_hazards(self, input=None, target=None, batch_ids=None, max_duration=None, sample=None,
        ...
        if baseline_hazards_ is None:
            baseline_hazards_ = self.compute_baseline_hazards(
                input, target, batch_ids, max_duration, sample, batch_size,
                set_hazards=False, eval_=eval_, num_workers=num_workers
                )
            
        # ------- Non-stratified (pd.Series_ --------
        ...
        # ---------- Stratified: (Dict) ----------
        if isinstance(baseline_hazards_, dict):
    ...
    def predict_surv_df(self, input, max_duration=None, batch_size=8224, verbose=False, baseline_hazards_=None,
        ...
        """
        cum_haz = self.predict_cumulative_hazards(input, max_duration, batch_size, verbose, baseline_hazards_,
                                                  eval_, num_workers)
