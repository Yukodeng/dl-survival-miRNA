...
class CoxPHStratified(_CoxPHBase):
    ...
    #     # Unpack the input and target data, separating out batch_indices
    #     input, target = data
    
    #     out = self.net(input)
    
    #     # Calculate metrics with only `targets` (excluding batch_indices)
    #     return {name: metric(out, *target) for name, metric in metrics.items()}
    
    def compute_loss(self, log_h, target):
        """Compute the stratified loss using the provided batch indices."""
        
        if self.batch_indices is None:
            raise ValueError("`batch_indices` is not set. Make sure to call `fit()` with `batch_indices`.")
        
        (durations, events) = target
        # Call the stratified loss function with batch_indices
        return self.loss(log_h, durations, events, self.batch_indices)
    ...
    def fit_dataloader(
        ...
        for _ in range(epochs):
            ...
            for data in dataloader:
                ...
                if stop:
                    break
                # unpack the input and target data 
                input, target = data
                
                self.optimizer.zero_grad() # zero out the gradients
                log_h = self.net(input) # forward
                
                self.batch_loss = self.compute_loss(log_h, target)
                
                # Set batch_metrics to include the batch loss for compatibility
                self.batch_metrics = {"loss": self.batch_loss}
    ...
    def fit(self, input, target, batch_indices=None, batch_size=256, epochs=1, callbacks=None, verbose=True,
            num_workers=0, shuffle=True, metrics=None, val_data=None, val_batch_size=8224,
            **kwargs):
        """Fit model with inputs, targets, and batch labels for stratification.
        
        Arguments:
            input {np.array, tensor or tuple} -- Input x passed to net.
            target {np.array, tensor or tuple} -- Target ([durations, events].
            batch_indices {np.array or torch.Tensor} -- Batch labels for stratification.
        ...
        # Ensure batch_indices is set before proceeding
        if self.batch_indices is None:
            raise ValueError("`batch_indices` must be provided either at initialization or during fit.")
